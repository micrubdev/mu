...
(defn- lsys-notes [rules axiom n]
  (map :note (vals-at (x/lsys rules axiom n) 0)))
...
(deftest lsys-handles-a-keyword-alphabet
  (testing "Lindenmayer's algae: a -> ab, b -> a"
    (let [algae {:a [:a :b], :b [:a]}]
      (is (= [:a :b]          (vals-at (x/lsys algae [:a] 1) 0)))
      (is (= [:a :b :a]       (vals-at (x/lsys algae [:a] 2) 0)))
      (is (= [:a :b :a :a :b] (vals-at (x/lsys algae [:a] 3) 0))))))
...
(deftest lsys-edges
  ...
  (testing "everything lands in one cycle, like sub and notes"
    (is (= [[0 1/3] [1/3 2/3] [2/3 1]]
           (->> (p/query (x/lsys fib-rules [0] 2) [0 1])
